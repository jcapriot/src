...
def sfaxa(fin,dim=1):
    ...
    axa=['n1','d1','o1']
    '''
    var=output('sfin %s.rsf'%fin+'|grep n%d | sed -e \"s/=/ /g\"'%dim)
    tmp=var.split()
...
def sfattr(fin,attr='min'):
    ...
    '''
    tmp=output('sfattr< %s.rsf'%fin+'|grep %s'%attr)
    tmp2=tmp.split()
    attr=tmp2[2]
    return attr
...
def rmbin(fin):
    ...
    '''
    var=output('sfin %s.rsf'%fin+'|grep in | sed -e "s/\\"/ /g"')
    tmp=var.split()
    cmd='rm -f %s'%tmp[2]
    os.system(cmd)
